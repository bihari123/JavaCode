...
class Labyrinth{
    ...
    static Lenkeliste<Lenkeliste<Route>> possible_routes ; 
    public Monitor all_possible_routes = new Monitor();
    protected int rows;
    protected int cols;
    private Labyrinth (int rows, int cols,Route[][] route){
        ...
        this.cols=cols;
        this.route=route;
        //all_possible_routes =  new Lenkeliste<Lenkeliste<Route>>();
    }
...
*/
    ...
    public void findExitFrom(int row, int col) {
      ...
      /*  try{
        while (all_possible_routes.stoerrelse()>0) {
           ...
           if(route[row][col] instanceof WhiteRoute || route[row][col] instanceof Opening ){
    ...
    */
    try{
        Lenkeliste<Route> way = new Lenkeliste<>();
		Lenkeliste<Route> start = new Lenkeliste<>();
		start.leggTil(route[row][col]);
        
        while(all_possible_routes.hentListe().stoerrelse()!=0){
            all_possible_routes.hentListe().fjern();
        }

		//Runnable task = new MyThread(start, vei); //Her er en oppgave
        Runnable Maintask=new MyThread(route[row][col], way);
        Thread mainThread = new Thread(Maintask); //Her er en arbeider
		mainThread.start(); //Leggo
		...
		Lenkeliste<Lenkeliste<Route>> r = all_possible_routes.hentListe();
        print("total number of routes are "+Integer.toString(r.stoerrelse())); 
		for(Lenkeliste<Route> v : r){
            //System.out.println(v.toString());
            for(Route elem:v){
                System.out.print(elem.coordToSring()+"--> ");
            }
            System.out.print("END \n \n");
		}
	
    }catch(IndexOutOfBoundsException e){
           print("pls insert a valid coordinate");
    } 
