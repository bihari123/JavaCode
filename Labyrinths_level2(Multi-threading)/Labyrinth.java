...
class Labyrinth {
    ...
    /*
     * A static factory method is a public static method on the object that returns
     * a new instance of the object. This pattern provides a layer between the
     * caller and the creation(factory)
     * 
     * Advantages: - Unlike constructors, they have names - Unlike Constructors,
     * they are not required to create a new object everytime they are invoked -
     * Unlike constructors, they can return an object of their return type -They
     * reduce the verbosity of creating parametrized type
     */
            ...
            for (int j = 0; j < labyrinth.route[0].length; j++) {
                if (labyrinth.route[i][j] instanceof WhiteRoute || labyrinth.route[i][j] instanceof Opening) {
                    if (i == 0) {
                        if (j == 0) {
                            if (labyrinth.route[i + 1][j] instanceof WhiteRoute) {
                                print(Integer.toString(i + 1) + "," + Integer.toString(j) + " is south to " + "("
                                        + Integer.toString(i) + "," + Integer.toString(j) + ")");
                                labyrinth.route[i][j].south = labyrinth.route[i + 1][j];
                            }
                            if (labyrinth.route[i][j + 1] instanceof WhiteRoute) {
                                print(Integer.toString(i) + "," + Integer.toString(j + 1) + " is east to " + "("
                                        + Integer.toString(i) + "," + Integer.toString(j) + ")");
                                labyrinth.route[i][j].east = labyrinth.route[i][j + 1];
                            }
                        } else if (j == labyrinth.route[0].length - 1) {
                            ...
                            if (labyrinth.route[i][j - 1] instanceof WhiteRoute) {
                                print(Integer.toString(i) + "," + Integer.toString(j - 1) + " is west to " + "("
                                        + Integer.toString(i) + "," + Integer.toString(j) + ")");
                                labyrinth.route[i][j].west = labyrinth.route[i][j - 1];
                            }
                        } else {
                        ...
                        if (j == 0) {
                            if (labyrinth.route[i - 1][j] instanceof WhiteRoute) {
                                print(Integer.toString(i - 1) + "," + Integer.toString(j) + " is north to " + "("
                                        + Integer.toString(i) + "," + Integer.toString(j) + ")");
    ...
    public Liste<String> findExitFrom(int row, int col) {
        ...
        try {
            while (all_possible_routes.stoerrelse() > 0) {
                ...
                print("clearing the all_possible_routes list");
                all_possible_routes.fjern();
            }
            print("calling find exit function");
            Thread t = new MyThread(route[row][col], "");
            t.start();
            try {
